...
$(document).ready(function () {
    ...
    function init() {
        ...
        // Создаем блоки с картами, каждая со своим числовым классом из массива objects b соответствующей картинкой
        for (let i = 0; i < allCards.length; i++) {
            $deck.append($('<div class="container"><div class="card ' + allCards[i] + '"><div class="front"><img src="pentagram-on.png" alt = "" /></div ><div class="back"><img src=" ' + allCards[i] + '.jpg" alt="" /></div></div ></div>'))
        }
        addCardListener();
    ...
    function gameOver(moves) {
        if (totalCard === match) {
            $('#winnerText').text(`You win! In ${MAX_SECONDS - second} seconds, you did a total of ${moves} moves. Well done!`);
    ...
    let addCardListener = function () {
        ...
        $deck.find('.card').on('click', function () {
            ...
            if ($this.hasClass('show') || $this.hasClass('match') || $this.hasClass('fliped')) { return true; }
